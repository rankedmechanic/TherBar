...
export default async function handler(req) {
  ...
  const provider = (
    req.headers.get('x-provider') || body._provider || 'anthropic'
  ).toLowerCase();
  ...
  if (provider === 'anthropic') {
    ...
    if (!key) return J(401, { error: 'No Anthropic API key. Add ANTHROPIC_API_KEY to env vars or supply one in Settings.' }, ch);
    ...
    if (!up.ok) return J(up.status, { error: d?.error?.message || up.statusText }, ch);
    return J(200, d, ch);
  }
  ...
  if (provider === 'openai') {
    ...
    if (!key) return J(401, { error: 'No OpenAI API key. Add OPENAI_API_KEY to env vars or supply one in Settings.' }, ch);
    ...
    const d = await up.json();
    if (!up.ok) return J(up.status, { error: d?.error?.message || up.statusText }, ch);
    // Normalise to Anthropic-style envelope
  ...
  if (provider === 'gemini') {
    ...
    if (!key) return J(401, { error: 'No Gemini API key. Add GEMINI_API_KEY to env vars or supply one in Settings.' }, ch);
