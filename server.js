...
async function handleProxy(req, res) {
  ...
  if (!rlOK(ip)) return sendJSON(res, 429, { error: 'Too many requests — please wait.' }, corsHeaders);

  // Read body
  const raw = await new Promise((ok, fail) => {
    let d = '';
    req.on('data', c => { d += c; if (d.length > 100_000) fail(new Error('Body too large')); });
    req.on('end', () => ok(d));
    req.on('error', fail);
  }).catch(() => null);
  ...
  let body;
  try { body = JSON.parse(raw); } catch { return sendJSON(res, 400, { error: 'Invalid JSON.' }, corsHeaders); }

  const provider = (req.headers['x-provider'] || body._provider || 'anthropic').toLowerCase();
  ...
  if (provider === 'anthropic') {
    ...
    if (!key) return sendJSON(res, 401, { error: 'No Anthropic API key. Set ANTHROPIC_API_KEY env var.' }, corsHeaders);
    ...
    let up;
    try {
      up = await httpsPost('https://api.anthropic.com/v1/messages',
        { 'x-api-key': key, 'anthropic-version': '2023-06-01' }, payload);
    } catch { return sendJSON(res, 502, { error: 'Could not reach Anthropic API.' }, corsHeaders); }

    const d = JSON.parse(up.body);
    if (up.status !== 200) return sendJSON(res, up.status, { error: d?.error?.message || 'Anthropic error' }, corsHeaders);
  ...
  if (provider === 'openai') {
    ...
    if (!key) return sendJSON(res, 401, { error: 'No OpenAI API key. Set OPENAI_API_KEY env var.' }, corsHeaders);
    ...
    let up;
    try {
      up = await httpsPost('https://api.openai.com/v1/chat/completions',
        { Authorization: `Bearer ${key}` }, payload);
    } catch { return sendJSON(res, 502, { error: 'Could not reach OpenAI API.' }, corsHeaders); }

    const d = JSON.parse(up.body);
    if (up.status !== 200) return sendJSON(res, up.status, { error: d?.error?.message || 'OpenAI error' }, corsHeaders);
    return sendJSON(res, 200, { content: [{ type: 'text', text: d.choices?.[0]?.message?.content || '' }] }, corsHeaders);
  }

  // ── Gemini ──
  if (provider === 'gemini') {
    const key = process.env.GEMINI_API_KEY || userKey;
    if (!key) return sendJSON(res, 401, { error: 'No Gemini API key. Set GEMINI_API_KEY env var.' }, corsHeaders);

    const parts = [];
    if (body.system) parts.push({ text: body.system + '\n\n' });
    (body.messages || []).forEach(m => parts.push({ text: (m.role === 'user' ? '' : 'Assistant: ') + m.content }));
    ...
    try {
      up = await httpsPost(
        ...
        {}, payload);
    } catch { return sendJSON(res, 502, { error: 'Could not reach Gemini API.' }, corsHeaders); }

    const d = JSON.parse(up.body);
    if (up.status !== 200) return sendJSON(res, up.status, { error: d?.error?.message || 'Gemini error' }, corsHeaders);
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/health' || url === '/api/health') {
    sendJSON(res, 200, { status: 'ok', version: '6.0.0', ts: new Date().toISOString() });
    return;
  }
...
server.listen(PORT, () => {
  ...
  console.log(`   ENV key: ${process.env.ANTHROPIC_API_KEY ? '✓ ANTHROPIC_API_KEY set' : '⚠ No env key — attorneys must enter their own'}`);
